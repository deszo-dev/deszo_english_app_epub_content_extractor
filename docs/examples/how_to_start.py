"""Minimal contract-first usage example for epub_content_extractor."""
...
with TemporaryDirectory() as tmp:
    ...
    result = extract_epub_content(input_path, config={})
    assert value(result, "schema_version") == "epub_content_extractor.v3.0"
    assert value(result, "status") == "succeeded"
    assert value(value(result, "book"), "language") == "en"
    assert len(value(value(result, "book"), "chapters")) == 1
